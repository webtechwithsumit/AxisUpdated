...
const OwnProduct = () => {
  ...
  const [columns, setColumns] = useState<Column[]>([
    ...
    { id: 'signedOffByDepartment', label: 'Signed Off By Department', visible: true },
      ...
      <Popover id={`popover-action-${item.id}`} className="shadow">
        <Popover.Body className="p-2">
          <Button variant="link" as={Link as any} to={`/pages/productMaster/${item.id}`} className="d-block text-start">
            <i className="ri-file-text-line me-2"></i> Document
          </Button>
          ...
          <Button variant="link" as={Link as any} to={`/pages/discussionForum/${item.id}`} className="d-block text-start text-green">
            <i className="ri-time-line me-2"></i> Assign Extension My Post
          </Button>
        </Popover.Body>
      </Popover>
    );
                ...
                <div className='bg-white p-2 pb-2'>
                  <Row className=''>
                    <div className="d-flex justify-content-between profilebar p-1">
                      <h4 className='text-primary d-flex align-items-center m-0'><i className="ri-file-list-line me-2 text-primary "></i> Own Department Product </h4>
